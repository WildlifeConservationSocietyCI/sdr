...
class Command(BaseCommand):
    ...
    def __init__(self):
        ...
        self.local_file_location = os.path.join(os.path.sep, 'tmp', 'sdr')
        try:
    ...
    def handle(self, *args, **options):
        ...
        if bucket_file_list and len(bucket_file_list) > 0:
            ...
            print('Retrieving latest back-up')
            ...
            # If the files doesn't exist locally, then download
    ...
    def _init_db(self):
        ...
        # for init, we assume admin PGUSER (postgres) etc is in env
        cmd = 'psql -h {db_host} -U {db_user}'.format(**params)
    ...
    def _psql_restore_db(self, file_name):
        ...
        self._run(command, to_file='/tmp/sdr/stdout_restore.log')
